				...
				<div class="panel panel-default">
					...
					<div class="container">
						<div class="col-md-8" style="margin-top: 20px;">			
							...
							<form action="${pageContext.request.contextPath}/process/save"
								class="form" method="post" role="form">
								
								<input value="${process.id}" type="hidden" name="id" />
								
								<div class="row">
									<div class="form-group">
										<label for="name">Processo</label> <input value="${process.name}" type="text"
									...
									<div class="form-group">
										...
										<select id="idEmployee" name="idEmployee" class="form-control">
									        <c:forEach var="employee" items="${employees}">
									            <option value="${employee.id}" ${employee.id == process.idEmployee ? 'selected="selected"' : ''}>${employee.name}</option>
									        </c:forEach>
									    </select>
							        </div>
								</div>
									...
									<div class="form-group">
							            ...
							            <select id="idCustomer" name="idCustomer" class="form-control">
									        <c:forEach var="customer" items="${customers}">
									            <option value="${customer.id}" ${customer.id == process.idCustomer ? 'selected="selected"' : ''}>${customer.name}</option>
									        </c:forEach>
									    </select>
							        </div>
								</div>
								<div class="row">
									<div class="form-group">
										...
										<textarea class="form-control" id="observation"
											name="observation">${process.observation}</textarea>
									</div>
								</div>
								<div class="row">
